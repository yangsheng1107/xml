...
static struct xml_string* xml_parse_content_with_cdata_format(struct xml_parser* parser) {
	xml_parser_info(parser, "content");
...
static struct xml_node* xml_parse_node(struct xml_parser* parser) {
	...
	if ('<' != xml_parser_peek(parser, CURRENT_CHARACTER)) {
	 ...
	 } else if ('!' == xml_parser_peek(parser, NEXT_CHARACTER)) {
		content = xml_parse_content_with_cdata_format(parser);
		if (!content) {
			xml_parser_error(parser, 0, "xml_parse_node::content");
			goto exit_failure;
		}
